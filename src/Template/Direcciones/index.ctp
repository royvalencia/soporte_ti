    ...
    <div class="row">
        <div class="col-lg-12">
          ...
          <?php foreach ($direcciones as $direccione): ?>
            <tr>
                <td><?= $this->Number->format($direccione->direccione_id) ?></td>
                <td><?= h($direccione->nombre) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('<i class="fa fa-th" ></i>'), ['action' => 'view', $direccione->direccione_id], ['class' => 'btn btn-default btn-sm','rel' => 'tooltip','title'=>'Ver','escape' => false]) ?>
                    <?= $this->Html->link(__('<i class="fa fa-edit" ></i>'), ['action' => 'edit', $direccione->direccione_id],['class' => 'btn btn-default btn-sm','rel' => 'tooltip','title'=>'Editar','escape' => false]) ?>
                    <?= $this->Form->postLink(__('<i class="fa fa-trash-o" ></i>'), ['action' => 'delete', $direccione->direccione_id], ['class' => 'btn btn-default btn-sm','rel' => 'tooltip','title'=>'Eliminar','escape' => false,'confirm' => __('¿Seguro que desea eliminar el registro # {0}?', $direccione->direccione_id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
              </tbody>  
