        ...
        <div class="col-lg-12">
          <div class="ibox float-e-margins">
               <div class="ibox-title">
                 <h5><?= __('Lista de Centros de Capturas') ?></h5>                    
               </div>               
            ...
            <table class="table table-striped table-hover table-index">
              <thead>
                <tr>                 
                                  ...
                                  <th><?= $this->Paginator->sort('iniciales') ?> <i class="fa fa-sort fa-1"></i></th>
                                <th class="actions"><?= __('Acciones') ?></th>
                </tr>
             </thead>
             <tbody>
          <?php foreach ($centrosCapturas as $centrosCaptura): ?>
            <tr>
                <td><?= $this->Number->format($centrosCaptura->id) ?></td>
                <td><?= h($centrosCaptura->descripcion) ?></td>
                <td><?= h($centrosCaptura->iniciales) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('<i class="fa fa-th" ></i>'), ['action' => 'view', $centrosCaptura->id], ['class' => 'btn btn-default btn-sm','rel' => 'tooltip','title'=>'Ver','escape' => false]) ?>
                    <?= $this->Html->link(__('<i class="fa fa-edit" ></i>'), ['action' => 'edit', $centrosCaptura->id],['class' => 'btn btn-default btn-sm','rel' => 'tooltip','title'=>'Editar','escape' => false]) ?>
                    <?= $this->Form->postLink(__('<i class="fa fa-trash-o" ></i>'), ['action' => 'delete', $centrosCaptura->id], ['class' => 'btn btn-default btn-sm','rel' => 'tooltip','title'=>'Eliminar','escape' => false,'confirm' => __('¿Seguro que desea eliminar el registro # {0}?', $centrosCaptura->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
              </tbody>  
