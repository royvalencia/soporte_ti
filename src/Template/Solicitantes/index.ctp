        ...
        <div class="col-lg-12">
          <div class="ibox float-e-margins">
               <div class="ibox-title">
                 <h5><?= __('Lista de Solicitantes') ?></h5>                    
               </div>               
          ...
          <?php foreach ($solicitantes as $solicitante): ?>
            <tr>
                <td><?= $this->Number->format($solicitante->solicitante_id) ?></td>
                <td><?= h($solicitante->nombre) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('<i class="fa fa-th" ></i>'), ['action' => 'view', $solicitante->solicitante_id], ['class' => 'btn btn-default btn-sm','rel' => 'tooltip','title'=>'Ver','escape' => false]) ?>
                    <?= $this->Html->link(__('<i class="fa fa-edit" ></i>'), ['action' => 'edit', $solicitante->solicitante_id],['class' => 'btn btn-default btn-sm','rel' => 'tooltip','title'=>'Editar','escape' => false]) ?>
                    <?= $this->Form->postLink(__('<i class="fa fa-trash-o" ></i>'), ['action' => 'delete', $solicitante->solicitante_id], ['class' => 'btn btn-default btn-sm','rel' => 'tooltip','title'=>'Eliminar','escape' => false,'confirm' => __('¿Seguro que desea eliminar el registro # {0}?', $solicitante->solicitante_id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
              </tbody>  
