        ...
        <div class="col-lg-12">
          ...
          <?php foreach ($estatus as $estatus): ?>
            <tr>
                ...
                <td><span class="label <?= $estatus->color ?>"><?= h($estatus->color) ?></span></td>
                <td class="actions">
                    <?= $this->Html->link(__('<i class="fa fa-th" ></i>'), ['action' => 'view', $estatus->id], ['class' => 'btn btn-default btn-sm','rel' => 'tooltip','title'=>'Ver','escape' => false]) ?>
                    <?= $this->Html->link(__('<i class="fa fa-edit" ></i>'), ['action' => 'edit', $estatus->id],['class' => 'btn btn-default btn-sm','rel' => 'tooltip','title'=>'Editar','escape' => false]) ?>
                    <?= $this->Form->postLink(__('<i class="fa fa-trash-o" ></i>'), ['action' => 'delete', $estatus->id], ['class' => 'btn btn-default btn-sm','rel' => 'tooltip','title'=>'Eliminar','escape' => false,'confirm' => __('¿Seguro que desea eliminar el registro # {0}?', $estatus->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
              </tbody>  
